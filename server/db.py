...
class Link(Base):
    ...
    last_checked = Column(
        DateTime, default=lambda: datetime.now(ZoneInfo("Asia/Ho_Chi_Minh"))
    )
...
def backup_old_table(engine, old_table_name, new_table_name):
    ...
    try:
        with engine.connect() as conn:
            ...
            conn.execute(
                text(f"ALTER TABLE {old_table_name} RENAME TO {backup_table_name}")
            )
            conn.commit()
...
def merge_old_table_data(engine, old_table_name, new_table_name):
    ...
    try:
        ...
        if not common_columns:
            logging.warning(
                f"No common columns found between {old_table_name} and {new_table_name}"
            )
            return False
        ...
        with engine.connect() as conn:
            ...
            conn.commit()
            logging.info(
                f"Migrated {result.rowcount} rows from {old_table_name} to {new_table_name}"
            )
            return True
...
def create_db_and_tables():
    ...
    database_url = os.environ.get("TURSO_DB_URL")
